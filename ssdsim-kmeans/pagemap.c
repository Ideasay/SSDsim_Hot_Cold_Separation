...
Status move_page(struct ssd_info * ssd, struct local *location, unsigned int * transfer_size)
{
    ...
    static unsigned int *Warm_Block = NULL;
	if(Hot_Block == NULL && Cold_Block == NULL && Warm_Block == NULL){
        ...
        Warm_Block = (unsigned int*)malloc(sizeof(int)*ssd->parameter->plane_die);
        *Hot_Block = NULL;
        *Cold_Block = NULL;
        *Warm_Block = NULL;
    }
    int sliceNum,clusterNum;
    ...
    //to find this lpn belongs to which slice
    sliceNum = transferLPN2Slice(lpn,ssd);
    //printf("slice num is %d\n",sliceNum);
    ////to find this slice belongs to which cluster
    clusterNum = sliceCenter[sliceNum];
    //printf("clusterNum num is %d\n",clusterNum);
    ...
    if(clusterNum == 0){
        #ifdef MPDEBUG
            printf("cluster1\n");//in fact, it's not comfirmed to be hot
        #endif
        ...
        {
            *(Hot_Block+location->plane) = get_new_block_Kmeans(ssd, location -> channel, location -> chip, location -> die, location -> plane);
        }
        ...
        ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);
            ...
            printf("Cluster2\n");
        #endif
        if( *(Warm_Block+location->plane) == NULL || ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[*(Warm_Block+location->plane)].free_page_num == 0)
        {
            *(Warm_Block+location->plane) = get_new_block_Kmeans(ssd, location -> channel, location -> chip, location -> die, location -> plane);
        }
        old_active_block = ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block;
        ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block = *(Warm_Block+location->plane);
        ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);
    }
        ...
        {
            *(Cold_Block+location->plane) = get_new_block_Kmeans(ssd, location -> channel, location -> chip, location -> die, location -> plane);
        }
        old_active_block = ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].active_block;
...
unsigned int get_new_block_Kmeans(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)
{
    unsigned int i;
    int block = -1;
    //Each plane corresponds to one hotness level
    for(i=0;i<ssd->parameter->block_plane;i++){
    ...
    if(block == -1){
        printf("get_new_block_kmeans Error\n");
        while(1){};
    }
    else return block;
}
