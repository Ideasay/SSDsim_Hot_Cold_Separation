...
int  main()
{
    ...
    unsigned  int i,j,k;
    struct ssd_info *ssd;
    /*hotnessMeta *hotnessData_ptr = (hotnessMeta *)malloc(sizeof(hotnessMeta) * N);
    memset(hotnessData_ptr,0, sizeof(hotnessMeta) * N);
    

    hotnessMeta hotnessData_try[5];
    
    for ( i = 0; i < 5; i++)
    {
        hotnessData_try[i].updateFreq = 1.3;
        hotnessData_try[i].updateInterval = 1.3;
    }

    for ( i = 0; i < 5; i++)
    {
        printf("init hotness data slice[%d],frq:%f,interval:%f\n",i,hotnessData[i].updateFreq,hotnessData[i].updateInterval);
    }
    
    
    
    //*hotnessData_ptr = {.updateFreq = 0,.updateInterval = 0};
    //kmeansTblInit(hotnessData);
    for (i = 0; i < N; i++)
    {
        //if the slice is never written in, freq shall set to 0 and interval should be TIME_WINDOW
        (hotnessData_ptr+i)->updateFreq = 1.3;
        //hotnessData[i].updateInterval = TIME_WINDOW;  
        (hotnessData_ptr+i)->updateInterval = 1.3;    
    }
    for ( i = 0; i < N; i++)
    {
        printf("init hotness data slice[%d],frq:%f,interval:%f\n",i,(hotnessData_ptr+i)->updateFreq,(hotnessData_ptr+i)->updateInterval);
    }*/
    for (i = 0; i < N; i++)
    {
        //if the slice is never written in, freq shall set to 0 and interval should be TIME_WINDOW
...
struct ssd_info *simulate(struct ssd_info *ssd)
{
    ...
    startKmeansTraining();//to get the begin time,auto set is 0s
    while(flag!=100)      
    {
        ...
        {   
            //printf("req_count: %d\n",req_count);
            //printf("flag: %d\n",flag);
            //in this tracefile,all are write reqs.so monitor all
            if(req_count < trainingCount)
            {
                //printf("trigger time: %lld\n",ssd->req_queue->time);
                if(ssd->request_queue->operation == 0)
                {
                    kmeansMonitoring(ssd->request_queue->lsn,ssd,req_count);
                    req_count++;
                }
            }
            else if(req_count == trainingCount)
            {
                //int currentTime = ssd->request_queue->time;
                //printf("current time:%lld\n",currentTime);
                //kmeansMonitoring(ssd->request_queue->lsn,ssd,req_count);
                for (int i = 0; i < N; i++)
                {
                    if (hotnessData[i].updateFreq >= 1)
                    {
                        hotnessData[i].updateInterval = endTime / hotnessData[i].updateFreq;
                    }
                    else
                    {
                        hotnessData[i].updateInterval = endTime;
                    }   
                }
                printf("dataset after processing:\n");
                for (int i = 0; i < N; i++)
                {
                    printf("hotness data slice[%d],frq:%f,interval:%f\n",i,hotnessData[i].updateFreq,hotnessData[i].updateInterval);
                }
